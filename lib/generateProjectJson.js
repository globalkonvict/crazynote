...
const generateProjectJson = async (config) => {
    const fileTree = await readFiles(config.directory, config);
    ...
    await fs.outputFile(outputPath, JSON.stringify(projectJson, null, 2));
...
const buildJsonStructure = (fileTree, rootDir, config) => {
    ...
    const addNode = (node, segments, file) => {
        if (segments.length === 0) {
            ...
            if (config.includeAST && (path.extname(file.path) === '.js' || path.extname(file.path) === '.jsx' || path.extname(file.path) === '.ts' || path.extname(file.path) === '.tsx')) {
